...
def _(np, plt):
    ...
    _fig  # Return figure for marimo to display
    return
...
def _(mo):
    ...
    - In this example, $u_k(x)$ does not change shape as $k$ changes. This is most applicable to lower energy electrons that are bound to the nucleus.
    """)
    return
...
def _(mo):
    ...
    - Restricting $ka \in (-\pi, +\pi]$ for the set of unique solutions,

        - If $N$ is odd, then $n=0, \pm 1, \pm 2, \ldots \pm (N-1)/2$. Thus, the number of discrete $k$ values is $N$.

        - If $N$ is even, then $n=0, \pm 1, \pm 2, \ldots +N/2$ (note that $n=-N/2$ leads to the same wavefunction as $n=+N/2$). The number of discrete $k$ values is also $N$.
...
def _(mo):
    ...
    _base_url = "https://joyce-poon.github.io/ECE350/energybands"
    ...
    mo.vstack(
        [
            ...
            mo.Html(f'<div style="text-align: center;"><img src="{_base_url}/KP_LHS.png" width="500" alt="Pierret Fig. 3.3"><p style="font-style: italic; color: #666;">Pierret Fig. 3.3</p></div>'),
            mo.md(
                r"""
               Energy Bands
                """
            ),
            mo.Html(f'<div style="text-align: center;"><img src="{_base_url}/KP_bands.png" width="500" alt="Pierret Fig. 3.5"><p style="font-style: italic; color: #666;">Pierret Fig. 3.5</p></div>'),
            mo.md(
                r"""
               E vs. k diagrams in reduced zone and extended zone representations, and in free-space
                """
            ),
            mo.Html(f'<div style="text-align: center;"><img src="{_base_url}/KP_extended_reduced_zone.png" width="600" alt="Pierret Fig. 3.6"><p style="font-style: italic; color: #666;">Pierret Fig. 3.6</p></div>'),
        ],
    )
    return
